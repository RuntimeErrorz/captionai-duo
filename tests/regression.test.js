...
assert.match(cueRecovery, /sendConfig\(\)/);
...
assert.match(background, /prompt-v22-context-budget/);
...
assert.match(background, /function repairSuspiciousSemanticTranslations/);
assert.match(background, /semantic-translation-repair-request/);
assert.match(background, /semantic-translation-repair-response/);
assert.match(background, /preservedChunkAlignment/);
assert.match(background, /repairId/);
assert.match(background, /repairedUnitTranslationsFromJsonText/);
...
assert.match(background, /cueId change is NOT a semantic boundary/);
...
assert.match(content, /DEEPSEEK_SEEK_BACKTRACK_ITEMS = 80/);
...
assert.match(content, /event\.type === "seeking"/);
