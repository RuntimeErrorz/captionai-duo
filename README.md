...
Both SSE network chunks and model tokens may split at arbitrary byte positions, so buffering happens in two layers: first through a complete blank-line-delimited SSE event, then through a complete top-level JSON object. Compact one-object-per-line JSONL remains the preferred model format, while framing also tolerates a provider splitting or pretty-printing one object across physical lines without weakening ordered structural validation. An alignment chunk inside an unfinished outer unit is never painted or committed; the complete outer unit must close before its semantic coverage can enter the streaming path. At flush, a missing outer wrapper is reconstructed only from complete alignment chunks; a missing or invalid ID still stops at the safe prefix. A half JSON string, half alignment chunk, or arbitrary token delta is never painted. Usage-only events are accumulated separately. After all current IDs are covered or `done` arrives, the extension allows at most 750ms for usage and `[DONE]`, then cancels an abnormal tail. A server that ignores SSE and returns ordinary JSON is accepted, including top-level `usage`; the previous complete-JSON schema remains a compatibility path.
