...
Both SSE network chunks and model tokens may split at arbitrary byte positions, so buffering happens in two layers: first through a complete blank-line-delimited SSE event, then through a complete top-level JSON object. Compact one-object-per-line JSONL remains the preferred model format, while framing also tolerates a provider splitting or pretty-printing one object across physical lines without weakening ordered structural validation. If a provider wraps many completed alignment chunks in one still-open outer unit, the parser validates and publishes each complete ordered chunk immediately; the eventual outer object is then de-duplicated against that streamed prefix. At flush, a missing outer wrapper is reconstructed only when the already-emitted alignment chunks are complete; a missing or invalid ID still stops at the safe prefix. A half JSON string, half alignment chunk, or arbitrary token delta is never painted. Usage-only events are accumulated separately. After all current IDs are covered or `done` arrives, the extension allows at most 750ms for usage and `[DONE]`, then cancels an abnormal tail. A server that ignores SSE and returns ordinary JSON is accepted, including top-level `usage`; the previous complete-JSON schema remains a compatibility path.
...
8. Overlapping YouTube cues can leave an independent semantic unit at a cue tail visible for only a few frames. When its effective window is under 650 ms and an adjacent unit belongs to the same raw cue, the display layer paginates and shows them together. AI boundaries, translation caches, and request payloads remain unchanged, so this costs no extra tokens; units from different raw cues are never joined by this rule.
