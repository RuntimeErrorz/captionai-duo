...
Pending coordinates near playback enter a bounded rolling window: the standard/DeepSeek cold window is 48 lexical items and continuous work uses about 80. A visible or newly sought target receives additional right-side semantic runway, followed by a separate private trailing guard, so the target is not placed directly against the guard and forced into a duplicate request. Gemini Flash Lite uses one wider approximately 320-item cold/steady/recovery window at every playback rate; character and semantic-boundary limits can still make an individual request smaller. Current-coordinate text also has an 18,000-character ceiling. Current text plus deduplicated context share an approximately 28,000-character budget; current content and the nearest context entries win when space is tight.
...
Gemini Flash Lite uses up to 320 current lexical items for urgent work at every playback rate; DeepSeek uses 64 at 1.75x and 48 at 2.5x. Gemini Flash Lite does not launch speculative future ranges, so the configured prefetch depth and DeepSeek's automatic look-ahead minimum do not consume extra requests. A complete outer JSONL unit can be published as soon as it closes, but alignment chunks inside an unfinished unit remain buffered; playback never treats a partial semantic unit as final. Once at least 64 safe streamed items remain ahead (16 at 2.5x and above) and the current response is near its end, the stream is cancelled at that safe cursor and an urgent continuation starts immediately; playback does not wait for the provider to finish an oversized tail, and the continuation can bypass speculative local slots. DeepSeek's future ranges remain bounded by its provider-specific local transport budget and the source-character limit; Gemini's visible lane is additionally limited to one active request per tab. Ordinary DeepSeek playback keeps the 160-item cap, while Gemini keeps the 320-item cap at every playback rate. At 2.5x and above, a stalled urgent request is rebuilt after about 3.5 seconds without a committed cursor advance (or 2.5 seconds after its last committed advance), rather than using the slower ordinary-playback grace. A speculative request that has passed its range without producing any committed stream progress for 2.5 seconds is cancelled and rebuilt around the visible target; a valid future request that already covers a reseeded cursor is promoted to the single visible writer instead of being discarded. If a provider emits malformed JSONL, the simpler fallback likewise returns a bounded 96-item prefix and exposes the remaining suffix for the next request. A new visible hard-boundary region cancels obsolete old-region writers before filling its runway. Gemini Flash Lite request starts are also spaced by about 4.2 seconds per endpoint/model, leaving a margin below the 15 RPM free-tier limit; this lowers RPM/RPD pressure but can make a fresh seek or a slow large request wait longer.

### 6. Playback, seek, prefetch, and concurrency

Visible playback work has priority, and future scopes are started according to Prefetch batches. DeepSeek normally allows up to two speculative requests plus the visible lane; at 1.75x and above this becomes three, and at 2.5x and above four. Generic compatible endpoints keep their provider-safe speculative lane. Gemini Flash Lite disables speculative prefetch, allows one non-urgent active request per tab, and spaces request starts globally for the configured endpoint/model. A request for the subtitle being watched or the new seek target is urgent and may bypass a limit already occupied by prefetch. Local slot pressure is not reported as a provider rate limit; provider `429` responses back off speculative scheduling instead of immediately replacing every failed request.
...
7. Pair source and translation pages by semantic order, then assign timed coordinates monotonically to page numbers. Playback can advance but does not oscillate between pages around adjacent cues. A short raw-cue gap inside the same semantic unit may be bridged only while it remains below the 4000 ms hard-pause threshold.
