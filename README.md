...
To avoid repeating verbose field names for every word, current coordinates are transmitted as compact rows: `[id, text]` when pause and boundary are both default, otherwise the trailing pause and boundary fields are appended only when needed. Context uses compact `[id, text]` rows. Full cue IDs and absolute timing remain local for validation, playback, pagination, and export.
