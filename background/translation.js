...
Stream one completed semantic unit per physical JSONL line. A unit line has exactly this shape: [[0,1,"..."],[2,2,"..."]]. The line itself is the chunk array; never add a type/chunks object wrapper. Each alignment chunk is exactly [start,end,translation]: start and end are numeric inclusive positions copied from CURRENT_CUES, zero-based and local to this request. Never enumerate an ids array or use a keyed chunk object. Each unit line must be independently valid, compact JSON on ONE line, with no Markdown fence, blank line, prefix or explanation. Emit a unit only after its complete sentence or clause is finalized; never revise an emitted unit later.
